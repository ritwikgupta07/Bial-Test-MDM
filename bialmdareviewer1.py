...
def main_app_logic():
    ...
    try:
        if not any(check_creds(c) for c in [AZURE_OPENAI_API_KEY_VAL, AZURE_OPENAI_ENDPOINT_VAL, SEARCH_QUERY_EMBEDDING_DEPLOYMENT_ID, AZURE_OPENAI_API_VERSION_VAL]):
            search_query_embeddings_model = AzureOpenAIEmbeddings(azure_deployment=SEARCH_QUERY_EMBEDDING_DEPLOYMENT_ID, azure_endpoint=AZURE_OPENAI_ENDPOINT_VAL, api_key=AZURE_OPENAI_API_KEY_VAL, api_version=AZURE_OPENAI_API_VERSION_VAL, chunk_size=1)
    except Exception as e: st.sidebar.error(f"Error initializing Embeddings Model: {e}")
    try:
        if not any(check_creds(c) for c in [AZURE_OPENAI_API_KEY_VAL, AZURE_OPENAI_ENDPOINT_VAL, AZURE_OPENAI_API_VERSION_VAL]):
            if not check_creds(PLANNING_LLM_DEPLOYMENT_ID): planning_openai_client = AzureOpenAI(api_key=AZURE_OPENAI_API_KEY_VAL, azure_endpoint=AZURE_OPENAI_ENDPOINT_VAL, api_version=AZURE_OPENAI_API_VERSION_VAL)
            if not check_creds(DEPLOYMENT_ID_VAL): synthesis_openai_client = AzureOpenAI(api_key=AZURE_OPENAI_API_KEY_VAL, azure_endpoint=AZURE_OPENAI_ENDPOINT_VAL, api_version=AZURE_OPENAI_API_VERSION_VAL)
    ...
    def get_query_plan_from_llm(user_question, client_for_planning):
        if not client_for_planning or check_creds(PLANNING_LLM_DEPLOYMENT_ID): return "Error: Planning LLM not configured.", None
...
Your JSON list of search queries:"""
        try:
            response = client_for_planning.chat.completions.create(model=PLANNING_LLM_DEPLOYMENT_ID, messages=[{"role": "user", "content": planning_prompt}], temperature=0.0, max_tokens=16000)
    ...
    def generate_answer_from_search(user_question, index_name, use_hybrid_semantic, vector_field, semantic_config, temperature, max_tokens_param, client_for_synthesis, show_details=True, system_prompt_override=None):
        if not client_for_synthesis or check_creds(DEPLOYMENT_ID_VAL): return "Error: Synthesis LLM not configured."
        ...
        if all_retrieved_details:
            unique_sources_dict = { (item.get('url') or item.get('filename_or_title')): item for item in all_retrieved_details }
            for i, item in enumerate(list(unique_sources_dict.values())):
                source_line = f"Source {i+1}: {html.escape(item['filename_or_title'])}"
                if item.get('url'): source_line += f" ([Link]({html.escape(item['url'])}))"
                if item.get('score') is not None: source_line += f" [Score: {item['score']:.4f}]"
                if item.get('reranker_score') is not None: source_line += f" [Reranker Score: {item['reranker_score']:.4f}]"
                formatted_sources.append(source_line)
        else:
            formatted_sources.append("No specific source metadata retrieved, but context was found.")
        formatted_refs_str = "\n".join(formatted_sources)

        if show_details: st.write("💡 Synthesizing final answer...")
        
        user_base_prompt_instructions = system_prompt_override or """Part 1:   "you are AI assistant for Multiyear tarrif submission for AERA. final response should have 1500 words at least.
...
Example 2: "For the true-up, the operator submitted actual passenger traffic of 45 million (as per Table 3.2), while the original figure considered by the Authority was 42 million (from Table 5.1 of the original Order)."""
        ...
        try:
            response = client_for_synthesis.chat.completions.create(model=DEPLOYMENT_ID_VAL, messages=[{"role": "user", "content": synthesis_prompt}], temperature=temperature, max_tokens=max_tokens_param)
            return response.choices[0].message.content
        except Exception as e: return f"Error generating synthesized answer: {e}"
    ...
    def refine_and_regenerate_report(original_report: str, new_info: str, client_for_synthesis) -> str:
        ...
        **FULL, REFINED, AND INTEGRATED REPORT:**"""
        with st.spinner("✨ Refining report with new information..."):
            try:
                response = client_for_synthesis.chat.completions.create(model=DEPLOYMENT_ID_VAL, messages=[{"role": "user", "content": refinement_prompt}], temperature=0.2, max_tokens=st.session_state.conv_agent_max_tokens)
    ...
    default_session_state = {
        ...
        "mda_analysis_type": "MDA Manpower Analysis", "mda_report_content": None,
        "mda_chat_history": [], "web_search_engine": "Bing" # Default search engine
    }
    ...
    with st.sidebar:
        ...
        with st.expander("⚙️ Settings & History", expanded=True):
            ...
            st.session_state.web_search_engine = st.radio("Web Search Engine", ("Bing", "Google (via Serper)"))
    ...
    if st.session_state.app_mode == "Conversational Agent":
        ...
        if st.session_state.last_retrieved_chunks_search:
            with st.expander("View Retrieved Context Sources", expanded=False):
                for item in st.session_state.last_retrieved_chunks_search:
                    st.markdown(f"**{html.escape(item.get('filename_or_title', 'Source'))}** ...")
        ...
        for i, q in enumerate(predefined_questions):
            if cols[i].button(q, key=f"predef_q_{i}"): st.session_state.question_text = q

        def update_question_text_conv():
            st.session_state.question_text = st.session_state.question_text_area_main_common
        st.text_area("Your question:", value=st.session_state.question_text, key="question_text_area_main_common", on_change=update_question_text_conv)
    ...
    "MDA Manpower Analysis": {
        ...
        if st.button("Generate Specific Analysis Report", type="primary", disabled=(not uploaded_word_file or not selected_specific_analysis_title)):
            ...
            with st.spinner("Processing document and generating analysis..."):
                if (extracted_text := extract_text_from_docx(uploaded_word_file)) and selected_specific_analysis_title:
                    ...
                    document_summary = synthesis_openai_client.chat.completions.create(model=DEPLOYMENT_ID_VAL, messages=[{"role": "user", "content": summary_prompt}]).choices[0].message.content
                    report_parts.append(f"### 1. Document Context (Generated Summary)\n{document_summary}")
                    
                    st.write(f"Step 2/2: Running analysis for '{selected_specific_analysis_title}'...")
                    ...
                    if prompt_template:
                        ...
                        analysis_answer = generate_answer_from_search(user_question=full_prompt, index_name=st.session_state.conv_agent_selected_index, use_hybrid_semantic=st.session_state.conv_agent_use_hybrid, vector_field=st.session_state.conv_agent_vector_field, semantic_config=st.session_state.conv_agent_semantic_config, temperature=st.session_state.conv_agent_temp, max_tokens_param=st.session_state.conv_agent_max_tokens, client_for_synthesis=synthesis_openai_client, show_details=False)
        ...
        if st.session_state.mda_report_content:
            ...
            if prompt := st.chat_input("Ask to elaborate or find new information..."):
                st.session_state.mda_chat_history.append({"role": "user", "content": prompt})
                ...
                with st.spinner("Agent is thinking..."):
                    ...
                    if is_web_search:
                        ...
                        if context_from_search.startswith("Error:"):
                            ...
                            web_search_prompt = f"""You are an expert AI assistant. Based on the following SEARCH RESULTS, provide a concise and informative answer to the user's question in 100 words.
                            USER QUESTION: "{prompt}"
                            SEARCH RESULTS:\n---\n{context_from_search}\n---\nYOUR DETAILED, FORMATTED ANSWER:"""
                            
                            response = synthesis_openai_client.chat.completions.create(model=DEPLOYMENT_ID_VAL, messages=[{"role": "user", "content": web_search_prompt}]).choices[0].message.content
                            answer = response
                    else:
                    ...
                    if answer:
                        st.session_state.mda_chat_history.append({"role": "assistant", "content": answer})
                        st.rerun()
